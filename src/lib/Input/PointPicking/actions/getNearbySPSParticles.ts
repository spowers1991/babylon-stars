...
export function getNearbySPSParticles(
  scene: BABYLON.Scene,
  camera: BABYLON.Camera,
  particlesInstance: BABYLON.SolidParticleSystem,
  onPicked: (particles: BABYLON.SolidParticle[]) => void
) {
  const threshold = 5;
  ...
  scene.onPointerObservable.add((pointerInfo) => {
    ...
    const candidates: { particle: BABYLON.SolidParticle; distanceAlongRay: number }[] = [];
